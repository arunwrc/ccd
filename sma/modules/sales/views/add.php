...
$(document).ready(function(){
	...
	var discount = <?php echo $discount_rate; ?>; var discount_type = <?php echo $discount_type; ?>;
	<?php } ?>
	...
	$('#code').keydown(function(e){
		...
		var item_code;
			...
			if(DISCOUNT_OPTION == 2) { echo '<td><select class="span2 select tran" data-placeholder="Select..." name="discount\' + count + \'" id="discount-\' + count + \'">';
			...
			echo '</select></td>'; } 
			if(TAX1) { echo '<td><select class="span2 select tran" data-placeholder="Select..." name="tax_rate\' + count + \'" id="tax_rate-\' + count + \'">';
	...
	$("#dyTable").on("click", '.del', function() {
		...
		select: function( event, ui ) {
			...
			echo '</select></td>'; } if(TAX1) { echo '<td><select class="span2 select tran" data-placeholder="Select..." name="tax_rate\' + count + \'" id="tax_rate-\' + count + \'">';
				foreach($tax_rates as $tax) {
					echo "<option value=" . $tax->id;
					if($tax->id == DEFAULT_TAX) { echo ' selected="selected"'; }
					echo ">" . $tax->name . "</option>";
...
<div class="control-group">
  ...
  <div class="controls">  <?php 
	 $bl[""] = "";
		...
		echo form_dropdown('biller', $bl, (isset($_POST['biller']) ? $_POST['biller'] : ""), 'id="biller_s" data-placeholder="'.$this->lang->line("select").' '.$this->lang->line("biller").'" required="required" data-error="'.$this->lang->line("biller").' '.$this->lang->line("is_required").'"'); ?> </div>
</div>
<!-- -->
<div class="control-group">
  <label class="control-label" id="biller_l"><?php echo $this->lang->line("on_credit"); ?></label>
  <div class="controls">
  <?php echo form_checkbox('on_credit', 'On Credit', FALSE);?>
  </div>
</div>
<!-- -->
<div class="control-group">
...
<div class="control-group">
  ...
  <div class="controls">  <?php 
	  $tr[""] = "";
		...
		echo form_dropdown('tax2', $tr, (isset($_POST['tax2']) ? $_POST['tax2'] : DEFAULT_TAX2), 'id="tax2_s" data-placeholder="'.$this->lang->line("select").' '.$this->lang->line("tax2").'" required="required" data-error="'.$this->lang->line("tax2").' '.$this->lang->line("is_required").'"'); ?> </div>
</div>
<?php } ?>
<?php if(DISCOUNT_OPTION == 1) { ?>
<div class="control-group">
