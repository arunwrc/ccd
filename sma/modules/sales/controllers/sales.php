...
redirect('module=home', 'refresh');
 ...
 $data['page_title'] = "Canceled_sales";
   ...
   function getdatatableajax()
   {
	   ...
	   $this->datatables
			->select("sales.id as sid, date, reference_no, on_credit, customer_name, user, total_tax2, total, internal_note")
			->from('sales');
...
//Add new invoice
   ...
   function add()
   {
	   $groups = array('purchaser', 'viewer');
		...
		$this->form_validation->set_rules('on_credit', $this->lang->line("on_credit"), 'xss_clean');
		...
		if ($this->form_validation->run() == true)
		{
			...
			$on_credit = $this->input->post('on_credit');
   ...
   function delete_delivery($id = NULL)
    {
        ...
        if($this->input->get('id')){ $id = $this->input->get('id'); } else { $id = NULL; }
			...
			{ 
				$this->session->set_flashdata('message', $this->lang->line("delivery_deleted"));
				redirect('module=sales&view=deliveries', 'refresh');
			} 
