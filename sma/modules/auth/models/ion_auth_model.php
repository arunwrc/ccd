...
class Ion_auth_model extends CI_Model
{
	...
	**/
	...
	public function updateUser($id, $email, $password, $additional_data = array(), $group_id) //need to test email activation
	{
		...
		if($password == NULL) {
			...
			$userData = array(
				...
				'company'      		=> $additional_data['company'],
				'phone' 			=> $additional_data['phone'],
				'default_warehouse' => $additional_data['warehouse'],
				'email'				=> $email,
	...
	public function get_total_results()
    {
	  ...
	  $this->db
			->select('products.id as productid, (CASE WHEN sum('.$this->db->dbprefix("warehouses_products.quantity").') Is Null THEN 0 ELSE sum('.$this->db->dbprefix("warehouses_products.quantity").') END) as totalQuantity, products.alert_quantity as alertQuantity', FALSE)
			->from('products')
