...
*/
	...
	function getdatatableajaxcost()
   {
	   ...
	   $this->datatables
			->select("products.id as productid, products.image as image, products.code as code, products.name as name, categories.name as cname, products.cost, products.price, warehouses_products.quantity as totalQuantity, products.unit, warehouses.name as warehouse_name")
			->from('products')
			...
			<a href='index.php?module=products&view=add_damage&product_id=$1' class='tip' title='".$this->lang->line("add_damage_qty")."'><i class='icon-filter'></i></a>
   ...
   function getdatatableajax()
   {
	   ...
	   $this->load->library('datatables');
	   $this->datatables
			->select("products.id as productid, products.image as image, products.code as code, products.name as name, categories.name as cname, products.price, (CASE WHEN sum(warehouses_products.quantity) Is Null THEN 0 ELSE sum(warehouses_products.quantity) END) as totalQuantity, products.unit, alert_quantity");
			...
			//->add_column("Barcode", "<img src='".$this->config->base_url()."index.php?module=products&view=barcode&code=$1' alt='$1' />", "code") 
			$this->datatables->add_column("Actions", 
			"<center><a id='$4 - $3' href='index.php?module=products&view=gen_barcode&code=$3&height=200' title='".$this->lang->line("view_barcode")."' class='barcode tip'><i class='icon-barcode'></i></a>
			<a href='#' onClick=\"MyWindow=window.open('index.php?module=products&view=product_details&id=$1', 'MyWindow','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=600,height=500'); return false;\" class='tip' title='".$this->lang->line("product_details")."'><i class='icon-fullscreen'></i></a>
			<a class='image tip' id='$4 - $3' href='".$this->config->base_url()."assets/uploads/$2' title='".$this->lang->line("view_image")."'><i class='icon-picture'></i></a>
   ...
   */
	...
	function getseatchresults($srq = NULL)
   {
	...
	function add()
	{
		...
		//validate form input
		$this->form_validation->set_rules('code', $this->lang->line("product_code"), 'trim|is_unique[products.code]|min_length[3]|max_length[50]|required|xss_clean');
	...
	function edit()
	{
		...
		//validate form input
		$this->form_validation->set_rules('code', $this->lang->line("product_code"), 'trim|min_length[3]|max_length[50]|required|xss_clean');
